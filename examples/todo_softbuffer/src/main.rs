            ...
            .with_fixed(
                TextInput::new("")
                    ...
                    .on_text_action(move |a| match a {
                        masonry::widgets::TextAction::Changed(e) => {
                            log::trace!("Changed input {e}")
                        }
...
fn main() {
    ...
    let owned_display_handler = event_loop.owned_display_handle();
    let context = Arc::new(softbuffer::Context::new(owned_display_handler).unwrap());
    velona::Builder::new(move |_| {
        VelloSoftbufferRenderer::new(
            context.clone(),
            SurfaceSettings {
